...
const normalizeUser = (user) => {
    const normalized = {
        ...
        id: user.id || user.login?.uuid || user.email, 
...
const finalizeUser = (user) => {
    ...
    const currentId = String(finalized.id || ''); 
    
    if (!finalized.id || currentId.includes('undefined')) {
        finalized.id = user.email || user.phone || `${Date.now()}-${Math.random()}`;
    }
    
    else {
        finalized.id = currentId; 
    }
    ...
    if (finalized.favorite === null || finalized.favorite === undefined) {
        finalized.favorite = Math.random() < 0.5;
    }
    if (!finalized.note) {
        finalized.note = '';
    }
...
export function mergeAndFormatUsers(mockUsers, additionalUsers) {
    ...
    allNormalizedUsers.forEach(user => {
        const primaryKey = user.email || user.full_name || user.id;
        ...
        let existingUser = Array.from(userMap.values()).find(u => 
            u.email === primaryKey || u.full_name === primaryKey || u.id === primaryKey
        );

        if (existingUser) {
            const merged = mergeUserData(existingUser, user);
            const mapKey = merged.email || merged.full_name || merged.id;
            userMap.set(mapKey, merged); 
        } else {
